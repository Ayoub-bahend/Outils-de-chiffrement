        ...
        for xor_key2 in keyRange:
            for xor_key3 in keyRange:
                for xor_key4 in keyRange:
                    for xor_key5 in keyRange:
                        key = bytes([xor_key1, xor_key2, xor_key3, xor_key4, xor_key5]) # 4 bytes key
                        # print(key)
                        ...
                        if ("ON" in decoded.decode() or "OFF" in decoded.decode()) and all (chr(decoded[i]) in MESSAGE_ALPHABET for i in range(len(decoded))):
...
if __name__ == "__main__":
    ...
    for i in range(1):
        ciphertext = "Z3BqBAFme3UDCw19fXR1FQVXBAYVBXgL"
